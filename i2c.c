...
void i2c_init(void)
{
    ...
    // making sure the default values are set correctly for I2C
    USII2C = 1;
    USICKPL = 1;
    USICKPH = 0
...
void i2c_start(void)
{
    // Send i2c START condition: forcing SDA low while SCL is on high
...
void i2c_stop(void)
{
	// Prepare i2c STOP condition while SCL is on high
    ...
    // Send i2c STOP condition by setting SDA high (from low)
