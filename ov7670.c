...
// set a register addr with value val
void ov7670_set(uint8_t addr, uint8_t val)
{
    i2c_start();              // start signal
    i2c_write8(OV7670_ADDR);  // transmit the address of the slave
    i2c_write8(addr);         // register address
    i2c_write8(val);          // set the register with val
    i2c_stop();               // stop signal
}

// receive the value of register addr
uint8_t ov7670_get(uint8_t addr)
{
    ...
    __delay_cycles(16 * 1000);  // wait 1 ms

    i2c_start();
    i2c_write8(OV7670_ADDR + 1);   // send the read address (42 + 1) = 43
...
// Setting the clock, VSYNC, camera settings registers
uint8_t ov7670_init(void)
{
    // if (ov7670_get(REG_PID) != 0x77) {
    //     return 0;
    // }
    ...
    ov7670_set(REG_COM10, 0x00);   // original 0x02
...
// Write a single image into FIFO memory  and reset the read pointer
void ov7670_capture(void)
{
    /* Capture frame */
    while ((P1IN & VSYNC));     // wait for an old frame to end
    while (!(P1IN & VSYNC));    // wait for a new frame to start, VSYNC to fall
    // P1OUT |= WEN;               // enable writing to fifo
    P1OUT &= ~WEN;
    while ((P1IN & VSYNC));     // wait for the current frame to end, VSYNC to rise
    P1OUT |= WEN;
    // P1OUT &= ~WEN;              // disable writing to fifo
    wait();
    P1OUT |= RRST;             // reset read pointer
}
